...
outcome:
  ...
  next: "The fitness criteria were spatial: sunlight, shade, open space, volume. The next version of this search should carry a carbon and a water number for each option, so the algorithm weighs the same things the climate plans do."
---
...
The studio brief was direct: urban growth will demand thousands of new cities, and the question
...
moves from a quarter to nearly half. The density map is a function of time rather than a fixed
zoning.
...
relationships between the two, which gene moves which criterion, are the design. Written this
way the intent can be argued with: you can disagree with a rule precisely, in a way you cannot
with a rendering.
...
loses on shade. There is no single best answer, only a set of compromises, and the designer chooses among them
with the reasons visible.
...
the grid, plates and cells that generate it drawn beside it. What looks like eight designs is
one intention, resolved eight ways.
...
the open space the ground could not. It is the earliest project on this site where the thing designed is a system rather than a
place.
